let usage_msg = "kernel3Par_run.exe <sparse_graph_input_file> <samples_input_file> -ndomains <num_domains>"
let files = ref []
...
let parellel_kernel3 pool graph samples res= 
    let len = Array.length res in
    T.parallel_for pool ~start:0 ~finish:(len-1) ~body:(fun i ->
        let g = graph in
        res.(i) <- (Kernel3Seq.kernel3 g samples.(i))
    ) 
...
let () =
    ...
    ignore @@ Sys.opaque_identity @@ T.run pool (fun () -> 
        parellel_kernel2 pool graph samples results);
